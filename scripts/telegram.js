...
listen(regexFactory.startsWith(["tg"]), function (match, data, replyTo, from) {

  var message = match[1].replace(/"/g, "''") || "";
  cmd = 'tmux send -t tg "msg Nurds '+message+'" ENTER';
  child = exec(cmd,                         // command line argument directly in string
    function (error, stdout, stderr) {      // one easy function to capture data/errors
      ...
      if (error !== null) {
        ...
        irc.privmsg('nooitaf', replyTo + ': ' + error);
        irc.privmsg('nooitaf', 'cmd: ' + message);
      } 
    }
  );
