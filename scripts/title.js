...
var db = require('./lib/listdb').getDB('links');
...
listen(/PRIVMSG [^ ]+ :.*?\b((?:https?:\/\/|www\d{0,3}[.]|[a-z0-9.\-]+[.][a-z]{2,4}\/)(?:[^\s()<>]+|\(([^\s()<>]+|(\([^\s()<>]+\)))*\))+(?:\(([^\s()<>]+|(\([^\s()<>]+\)))*\)|[^\s`!()\[\]{};:'".,<>?]))/i, function(match, data, replyTo, from) {
    ...
    console.log('title: Found url_2: "' + url2 + '" ' + url2.length + ' "'+url2[url2.length-1]+'" charcode: ' + url2.charCodeAt(url2.length-1));
    ...
    request({url: url, method: 'HEAD'}, function (error, headRes) {
        ...
        req.on('data', function (data) {
            ...
            if(titleMatch && titleMatch[1]) {
                ...
                // irc.privmsg(replyTo, "^ " + title, false);
            }
        });
